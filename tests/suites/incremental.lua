...
function main(xrefl, opt)
    ...
    print("-- edit a helper beside the emitter --")
    _settle()
    io.writefile(path.join(work, "tools", "names.lua"), "function suffix() return '' end\n")
    _check(tally, "every header is reparsed when a helper module appears", 2, _build(work))
    _settle()
    io.writefile(path.join(work, "tools", "names.lua"), "function suffix() return '!' end\n")
    _check(tally, "every header is reparsed when a helper module changes", 2, _build(work))
